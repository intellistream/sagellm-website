...
def normalize_entry(entry: Dict) -> Dict:
    """Normalize legacy field names to current leaderboard schema."""
    hardware = entry.get("hardware", {})

    interconnect = hardware.get("interconnect")
    intra_node_interconnect = hardware.get("intra_node_interconnect")

    if not interconnect and intra_node_interconnect:
        hardware["interconnect"] = intra_node_interconnect
    elif not intra_node_interconnect and interconnect:
        hardware["intra_node_interconnect"] = interconnect
    ...
    for json_file in results_dir.rglob("*_leaderboard.json"):
        try:
            with open(json_file, "r", encoding="utf-8") as f:
                data = json.load(f)
                all_results.append(normalize_entry(data))
...
def main():
    ...
    print("\n✅ 聚合完成！")
    print(
        f"  📄 {single_file.name}: {len(single_data)} 条 (单机单卡: {len(single_chip)}, 单机多卡: {len(multi_chip)})"
    )
