...
- All release banner, Quick Start copy, and package versions are managed in `data/version_meta.json`.
- Do not edit release/quickstart copy directly in `README.md` for normal updates.

## Update Flow

1. Edit `data/version_meta.json` (copy fields under `release` / `quickstart`, package list under `packages`).
2. Run sync script:
...
3. Validate stale/version consistency:

   ```bash
   bash scripts/check_stale_versions.sh
   ```

4. Commit generated updates (`data/version_meta.json`, `README.md`).
